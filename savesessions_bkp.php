<?php
	...
	foreach ($cursos as $c => $curso){
		...
		if (count($bicJr) > 0){ 
			...
			if( $nResArea > 7){
				$nResInseridos = 7;
				$ideal = (int) ($nResArea/7); // numero de sessoes que a area ira ocupar, tomando 7 como ideal de resumos por sessao
				$trataResto = 1;
			}else{
			...
			if ($trataResto == 1){//verifica se precisa tratar o resto ou nao ( possui menos de 7 resumos total, vide linhas 76 a 83 )
				$resto = $nResArea%7;
				$sec = (0 + $encheu);
				for($j=0; $j< $resto; $j++){
