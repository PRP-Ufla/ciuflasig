<?php
	...
	$db = new Database();
	$id = $_GET['id'];
	$sql = "SELECT * FROM resumos_recusados WHERE id='".$id."';";
	$res = $db->consulta($sql);
	if (count($res) == 0)
	...
	echo "<tr><th>Parecer da Avaliação:</th><td><br>".
		$data['parecer_avaliacao']."</td></tr>";
	
	echo "</table><br />";
	
	echo "<button onClick='resumos_recusados();'>&lt;&lt; Voltar</button>";
?>