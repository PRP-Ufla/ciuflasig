<?php
	...
	$db = new Database();
	$sql = "SELECT * FROM resumos_recusados INNER JOIN eventos ".
		"ON resumos_recusados.eventos_id=eventos.id WHERE resumos_recusados.id='".
		$_GET['id']."';";
	$res = $db->consulta($sql);
	if (count($res) == 0)
	...
	$parecer_avaliacao = str_replace("\n", " ",str_replace($busca,$troca,html_entity_decode( $data['parecer_avaliacao'],ENT_QUOTES, 'ISO-8859-15')));
	...
	$pdf->Ln(5);
	$pdf->SetFont('Helvetica','B',11);
	$pdf->Cell(0,9,"Parecer da Avaliação",0,0,"L");
	$pdf->Ln(10);
	$pdf->SetFont('Helvetica','',11);
	$pdf->MultiCell(0,4.5,$parecer_avaliacao);
