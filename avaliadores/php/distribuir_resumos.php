<?php
	...
	$dados = mysql_query('	SELECT r.sessoes_id, r.cursos_id, r.num_resumos, a.num_avaliadores, a.avaliadores_id, r.resumos_id, r.nomes_1autor, v.disponiveis, 
								...
								(SELECT re.sessoes_id, re.cursos_id, re.eventos_id, GROUP_CONCAT(re.id ORDER BY re.autor1) resumos_id, GROUP_CONCAT(re.autor1 ORDER BY re.autor1) nomes_1autor, COUNT( * ) num_resumos

								FROM  

								/* sub-seleção para ordenar os resumos por evento, sessao e nome do 1 autor*/
								(SELECT * FROM resumos ORDER BY eventos_id,sessoes_id,cursos_id,autor1)re 
	...
	*/
			...
			for($x = 0; $x < $numAvaliadores[$i]; $x++ ){
				...
				if($restob != 0){
					...
					for($y = 0; $y <= $resumos_por_avaliador[$i];  $y++ ){
								...
								if (!$db->executar($alterarAvaliador)) echo 2;
								$conta++;
							$contador++;
						}
	...
	/* Descrição dos echos:
		1 significa sucesso ao distrubuir
		2 significa falha*/
