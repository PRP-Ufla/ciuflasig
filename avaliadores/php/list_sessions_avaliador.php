<?php
	...
	if (count($res) == 0)
		...
		foreach ($res as $s => $ses) {
			...
			$msg .= "<hr /><h3 style='background-color: #D3D3D3; text-align:center; font-size: 30px;'>SESS&Atilde;O DE P&Ocirc;STERES ".$ses['id']."</h3>";
			...
			foreach ($resumos as $r => $resum) {
				if ($resum['cursos_id'] != $lastCurso) {
					...
					$msg .= "<table border=1 width = 100% style='font-size: 14px;'><tr><th>".
						...
						"</th><th>T&iacute;tulo:</th><th>Avaliador</th></tr>";
				}
				$msg .= "<tr>";
				$msg .= "<td>".($r+1)."</td>";
				$msg .= "<td>".$resum['autor1']."</td>";
				$msg .= "<td>".$resum['titulo']."</td>";
				$msg .= "<td>".$resum['avaliador']."</td>";
				$msg .= "</tr>";				
			}
		}else{
